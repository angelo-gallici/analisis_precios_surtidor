...
function construirFiltro() {
  const productos = new Set(geojsonDatos.features.map(f => f.properties.producto));
  const filtroDiv = document.getElementById("filtro-productos");

  productos.forEach(prod => {
    const label = document.createElement("label");
    label.innerHTML = `
      <input type="checkbox" value="${prod}"> ${nombresBonitos[prod.toLowerCase()] || prod}
    `;
    filtroDiv.appendChild(label);
  });

  filtroDiv.addEventListener("change", () => {
    actualizarVista();
    buscarMejorEstacion();
  });
}
...
function construirSlider() {
  ...
  const label = document.getElementById("radio-valor");

  if (slider && label) {
    slider.addEventListener("input", () => {
      label.innerText = `${slider.value} km`;
      if (ubicacionUsuario) {
        dibujarCirculo(parseFloat(slider.value));
        buscarMejorEstacion();
      }
    });
  }
...
function obtenerUbicacion() {
  ...
  navigator.geolocation.getCurrentPosition(
    pos => {
      ...
      dibujarCirculo(parseFloat(document.getElementById("radio-slider").value));
      buscarMejorEstacion();
    },
    err => {
      console.warn("No se pudo obtener la ubicación:", err.message);
...
function actualizarVista() {
  const seleccionados = Array.from(document.querySelectorAll("#filtro-productos input:checked")).map(e => e.value);
  ...
  geojsonDatos.features.forEach(f => {
    const prod = f.properties.producto;
    if (seleccionados.includes(prod)) {
      const key = `${f.properties.bandera.toUpperCase()}|${f.geometry.coordinates[1]}|${f.geometry.coordinates[0]}|${f.properties.direccion}`;
      if (!estacionesMap.has(key)) {
        estacionesMap.set(key, {
          bandera: f.properties.bandera.toUpperCase(),
          latlng: [f.geometry.coordinates[1], f.geometry.coordinates[0]],
...
function buscarMejorEstacion() {
  const seleccionados = Array.from(document.querySelectorAll("#filtro-productos input:checked")).map(e => e.value);
  if (!ubicacionUsuario || seleccionados.length === 0) return;

  const radioKm = parseFloat(document.getElementById("radio-slider").value);
  ...
  geojsonDatos.features.forEach(f => {
    const prod = f.properties.producto;
    if (!seleccionados.includes(prod)) return;

    const [lon, lat] = f.geometry.coordinates;
    const distancia = mapa.distance(ubicacionUsuario, [lat, lon]);

    if (distancia > radioM) return;

    const key = `${lat},${lon}`;
    if (!mejor || f.properties.precio < mejorScore) {
      mejor = f;
      mejorScore = f.properties.precio;
    }
  });

  if (mejor) {
    const [lon, lat] = mejor.geometry.coordinates;

    if (capaMejorEstacion) {
      mapa.removeLayer(capaMejorEstacion);
    }

    capaMejorEstacion = L.marker([lat, lon], {
      icon: L.divIcon({
        html: `<div style="background-color:#0f0;border:2px solid #080;padding:4px;border-radius:50%;width:28px;height:28px;"></div>`,
        iconSize: [28, 28],
